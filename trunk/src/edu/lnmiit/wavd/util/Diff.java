...
public class Diff {
    ...
    private static List getEdits(int srcStart, int srcEnd, CharSequence src, int dstStart, int dstEnd, CharSequence dst) {
        ...
        // lcs.getLength()) + "'");
        if (lcs.getLength() > 0) {
            edits.addAll(getEdits(srcStart, lcs.getSrcLocation(), src, dstStart, lcs.getDstLocation(), dst));
            srcStart = lcs.getSrcLocation() + lcs.getLength();
            dstStart = lcs.getDstLocation() + lcs.getLength();
            edits.addAll(getEdits(srcStart, srcEnd, src, dstStart, dstEnd, dst));
        } else {
            edits
                    .add(new Edit(srcStart, src.subSequence(srcStart, srcEnd), dstStart, dst.subSequence(dstStart,
                            dstEnd)));
        }
        return edits;
    }
     ...
     * @return the edits
     */
    private static List getEdits(int srcStart, int srcEnd, CharSequence[] src, int dstStart, int dstEnd,
            CharSequence[] dst) {
        List edits = new LinkedList();
    ...
    public static class Edit {
         ...
         */
        public String toString() {
            return srcLocation + "- '" + src + "', " + dstLocation + "- '" + dst + "'";
        }
    }
